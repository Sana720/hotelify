    ...
    useEffect(() => {
        async function fetchRoomTypes() {
            ...
            const { data } = await supabase
                .from('room_types')
                .select('id, name, base_price, amenities')
                .eq('org_id', tenant.id);
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-black tracking-tight text-white uppercase italic">Inventory <span className="text-blue-500">Expansion</span></h1>
                    <p className="text-zinc-500 font-bold mt-1 uppercase tracking-widest text-[9px]">Map physical rooms to their respective categories.</p>
                </div>
                <div className="flex items-center gap-3">
                    <Dialog open={isOpen} onOpenChange={setIsOpen}>
                        <DialogTrigger asChild id={`${roomDialogId}-trigger`}>
                            <Button className="bg-white text-black hover:bg-zinc-200 h-10 px-6 rounded-xl gap-2 font-black uppercase tracking-widest text-[9px] shadow-lg shadow-white/5 transition-all">
                                <Plus className="w-4 h-4" />
                                New Room
                            </Button>
                        </DialogTrigger>
                        <DialogContent id={`${roomDialogId}-content`} className="glass-premium border-white/10 bg-[#0a0a0c]/95 backdrop-blur-3xl text-white rounded-[2.5rem] p-8 max-w-lg">
                            <DialogHeader>
                            ...
                            <form onSubmit={handleAddRoom} className="space-y-6 mt-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            className="h-10 bg-white/[0.03] border-white/5 rounded-xl font-bold transition-all focus-visible:!ring-2 focus-visible:!ring-blue-500/20 focus-visible:!border-blue-500"
                                            required
                                        />
                                    </div>
                                    <div className="space-y-2">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Select
                                        ...
                                        onValueChange={v => {
                                            const selectedType = roomTypes.find(t => t.id === v);
                                            setFormData({ 
                                                ...formData, 
                                                room_type_id: v,
                                        ...
                                        required
                                    >
                                        <SelectTrigger className="h-12 bg-white/[0.03] border-white/5 rounded-xl font-bold">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="grid grid-cols-3 gap-3 max-h-[120px] overflow-y-auto pr-2 custom-scrollbar">
                                                ...
                                                <div
                                                    ...
                                                    className={`flex items-center gap-2 p-2 rounded-xl border transition-all ${
                                                        isActive
                                                            ...
                                                            : 'bg-white/[0.03] border-white/5 text-zinc-500/40 opacity-40 grayscale'
                                                    }`}
                                                >
                                                    {getIcon(amenity.icon)}
                                                    <span className="text-[8px] uppercase tracking-widest leading-none">{amenity.name}</span>
                                                </div>
                                            );
                                        })}
                                    </div>
                                ...
                                <DialogFooter className="mt-8">
                                    <Button
                                        ...
                                        className="w-full h-12 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-black uppercase tracking-widest text-[10px] transition-all shadow-lg shadow-blue-500/20"
                                    >
